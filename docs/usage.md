...
Your HelloImg API key is issued for one website and only works from that
site (subdomains included) — every request carries the site's host, and
the live API rejects calls from anywhere else.
