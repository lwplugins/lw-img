...
/**
 * The convert target must match each given file's own format exactly —
 * crop() derives it per job from the job's file name, since a size can
 * legitimately differ from the main file on WordPress 7.1 — and refuse
...
final class ThumbnailCropperTest extends MonkeyTestCase {
	 ...
	 */
	public function test_convert_target_matches_the_given_files_own_format( string $file, ?string $expected ): void {
	...
	public static function provide_extensions(): array {
		return [
			...
			'size-file basename, as fed per job from metadata' => [ 'photo-150x150.webp', 'webp' ],
